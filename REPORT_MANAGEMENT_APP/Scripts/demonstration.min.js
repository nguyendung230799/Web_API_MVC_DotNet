...
function orderRandom() {
    var eb = $(".stats .icon-file-alt").parent().find(".details .big"),
        es = $(".stats .icon-file-alt").parent().find(".details .small"),
        t = parseInt(eb.html()),
        n = Math.random() * 10,
        r = 1;
    n >= 5 && (r = -1);
    ...
    $.get("/api/Me/GetReportSize", function (data) {
        eb.html(`${data.fileCount} báo cáo`);
        es.html(`Dung lượng ${data.fileSize}MB`);
    });
...
function currentTime() {
    ...
    e.find(".details .big").html(
        "Ngày " + t.getDate() + " Tháng " + n[t.getMonth()] + ", " + t.getFullYear()
    );
    e.find(".details span")
        .last()
        .html(
